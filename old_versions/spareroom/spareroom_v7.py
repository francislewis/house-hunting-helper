...
def search_for_spareroom_ids(preferences, max_prop_to_return=200):
    """
    Search according to preferences and return list of spareroom ids.
    :param preferences: dict
    :param max_prop_to_return:
    :return:
    """
    ...
    pages_left = ceil(min(max_prop_to_return / params['max_per_page'], total_pages))
...
def getspareoominfo(room_id, sleep_time=1):
    ...
    spareroom_prop = {'id': sr_prop['advert_summary']['advert_id'], 'title': sr_prop['advert_summary']['ad_title'],
                      ...
                      'available_from': sr_prop['advert_details'][0]['content'][5]['Available'],
                      'EPC': 'N/A',
                      'has_garden': False,
                      ...
                      'Minimum Tenancy': sr_prop['advert_details'][0]['content'][6]['Min term'],
                      'Bills Included': False,
                      'nearest_station': sr_prop['advert_details'][0]['content'][4]['Nearest Station'],
...
list_of_ids = search_for_spareroom_ids(preferences, max_prop_to_return=20)
for id in list_of_ids:
    details = getspareoominfo(id)
    write_property_details_to_db(details)
